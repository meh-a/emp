...
function drawBuildingObj(b, sz) {
  ...
  if (!b.complete) {
    ...
    // Tier badge
    if (b.complete && (b.tier||1) > 1 && zoom >= 22) {
      const badge = b.tier === 3 ? 'III' : 'II';
      const fs = Math.max(6, Math.floor(zoom * 0.18));
      ctx.font = `bold ${fs}px 'Silkscreen',monospace`;
      ctx.textAlign = 'center'; ctx.textBaseline = 'top';
      ctx.fillStyle = '#f5d080';
      ctx.fillText(badge, sx + bw * 0.5, sy + 2);
    }
  }
}
...
function drawNPCChar(npc, px, py, sz) {
  ...
  if (sz >= 42) {
    ...
    let label;
    if (npc.type === 'trader') label = npc.caravan ? `🛒 ${npc.name}` : `⚜ ${npc.name}`;
    else                       label = `⚔ ${npc.name}`;

...
function tileBaseColor(tx, ty, t, v, time) {
  ...
  switch(t) {
    ...
    case T.GRASS: {
      const GRASS_RGB = [[72,168,48],[70,158,38],[130,130,45],[105,120,95]];
      const [gr,gg,gb] = GRASS_RGB[season] ?? GRASS_RGB[0];
      return tint(gr,gg,gb,s, v*0.15-0.075);
    }
    case T.FOREST: {
      const FOREST_RGB = [[26,88,16],[22,80,14],[60,70,20],[18,40,22]];
      const [fr,fg,fb] = FOREST_RGB[season] ?? FOREST_RGB[0];
      return tint(fr,fg,fb,s, v*0.12-0.060);
    }
...
// Precompute colors for every non-animated tile — called once after world gen.
// Water/deep/river tiles are animated (time-dependent) so they stay null.
function buildTileColorCache() {
  _tileColorCache = new Array(MAP_H * MAP_W);
  _lastCachedSeason = season;
...
function drawSnowOverlay() {
  ...
  for (let i = 0; i < 110; i++) {
    const px = ((i * 173.7 + time * (7 + (i % 5) * 2.3)) % W + W) % W;
    const py = ((i * 97.1  + time * (13 + (i % 7) * 2.8)) % H + H) % H;
    const r  = 0.9 + (i % 3) * 0.65;
    ctx.beginPath();
...
function drawGroundBuildingInline(b, sx, sy, sz) {
  ...
  // Tier badge for T2/T3 ground buildings (farmland, mine)
  const _tier = b.tier || 1;
  if (b.complete && _tier >= 2 && sz >= 16) {
    const label = _tier === 3 ? 'III' : 'II';
    const fs = Math.max(6, Math.floor(sz * 0.14));
    ctx.font = `bold ${fs}px 'Silkscreen',monospace`;
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    const bx = Math.floor(sx + sz - fs * 0.1);
    ...
    ctx.fillStyle = _tier === 3 ? 'rgba(120,60,180,0.88)' : 'rgba(180,120,20,0.88)';
    ctx.beginPath(); ctx.roundRect(rx, ry, rw, rh, Math.floor(rh * 0.35)); ctx.fill();
    ctx.fillStyle = _tier === 3 ? '#e0b8ff' : '#ffe090';
...
// ═══════════════════════════════════════════════════
const NODE_ICON = { quarry:'pickaxe', iron:'iron', farmland:'leaf', forest:'pine', delta:'drop' };
const NODE_RING_COL = {
...
function drawResourceNodes(sz, c0, r0, c1, r1) {
  ...
  for (const node of resourceNodes) {
    ...
    if (sz >= 16) {
      ...
      const iconName = NODE_ICON[node.type];
      if (iconName) {
        drawIcon(ctx, iconName, cx, cy, Math.round(fs));
      } else {
...
function drawMinimap() {
  ...
  // Own TC dot
  if (townCenter) {
    const px = townCenter.tx / MAP_W * mw, py = townCenter.ty / MAP_H * mh;
    mmCtx.fillStyle = _kingdomColor(myKingdomId);
    mmCtx.fillRect(Math.floor(px)-2, Math.floor(py)-2, 5, 5);
  }
  // Enemy TC dots
  for (const ek of enemyKingdoms) {
    ...
    if (!fogExplored[fi]) continue;
    const px = ek.tx / MAP_W * mw, py = ek.ty / MAP_H * mh;
    mmCtx.fillStyle = ek.isPlayer ? _kingdomColor(ek.id) : 'rgba(200,60,60,0.9)';
    mmCtx.fillRect(Math.floor(px)-2, Math.floor(py)-2, 4, 4);
  }
...
function render() {
  ...
  if (_lastCachedSeason !== season) buildTileColorCache();
  ...
  for (let row=r0; row<=r1; row++) {
    for (let col=c0; col<=c1; col++) {
      ...
      const _tc = _tileColorCache?.[row * MAP_W + col];
      ctx.fillStyle = _tc !== undefined ? _tc : tileBaseColor(col, row, t, v, time);
  ...
  if (particles.length) {
    ...
    for (const p of particles) {
      const sx = p.x * psz - camX;
      const sy = p.y * psz - camY;
      const pw = Math.max(2, p.size * psz);
      ctx.globalAlpha = Math.max(0, p.life);
      ctx.fillStyle = p.color;
      ctx.fillRect(sx - pw * 0.5, sy - pw * 0.5, pw, pw);
    }
    ctx.globalAlpha = 1.0;
  }

  // Loot drop coins flying to HUD gold counter
  if (lootDrops.length) {
    const goldEl = document.getElementById('res-gold');
    const rect = goldEl ? goldEl.getBoundingClientRect() : null;
    const tx = rect ? rect.left + rect.width * 0.5 : canvas.width * 0.5;
    const ty = rect ? rect.top  + rect.height * 0.5 : 24;
    ctx.font = 'bold 13px Silkscreen,monospace';
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    for (const d of lootDrops) {
      const progress = Math.min(d.t / 1.0, 1.0);
      const floatEnd = 0.32;
      let cx, cy;
      if (progress < floatEnd) {
        const fp = progress / floatEnd;
        cx = d.sx;
        cy = d.sy - fp * 28;
      } else {
        const fp = (progress - floatEnd) / (1.0 - floatEnd);
        const ease = fp * fp * (3 - 2 * fp); // smoothstep
        const startX = d.sx, startY = d.sy - 28;
        cx = startX + (tx - startX) * ease;
        cy = startY + (ty - startY) * ease;
      }
  ...
  // Vignette — gradient cached until canvas resizes
  if (!_vigGrad || canvas.width !== _vigW || canvas.height !== _vigH) {
    _vigW = canvas.width; _vigH = canvas.height;
    _vigGrad = ctx.createRadialGradient(_vigW*.5,_vigH*.5,_vigH*.28,_vigW*.5,_vigH*.5,_vigH*.88);
    _vigGrad.addColorStop(0,'rgba(0,0,0,0)');
    _vigGrad.addColorStop(1,'rgba(0,0,0,0.38)');
  }

...
// ── Per-kingdom colors (index by kingdom.id) ─────────────────────────
...
function _kingdomColor(id) { return KINGDOM_COLORS[(id ?? 0) % KINGDOM_COLORS.length]; }
...
const CHOP_TIME    = 14;    // seconds per tree chop
...
// ── Gameplay constants ──────────────────────────────
const FARM_TIME        = 24;   // seconds per farming cycle (modified by soil fertility 1–3)
...
const MINE_TIME        = 26;   // seconds per mining cycle (reduced by mountain elevation bonus)
...
const TIER_XP_REQ   = [10, 30, 60, 100];           // T1→T2, T2→T3, T3→T4, T4→T5
const TIER_SPEED    = [1.0, 1.15, 1.35, 1.55, 1.80]; // work-speed multiplier per villager tier
// Tool inventory
let toolStock = [999, 0, 0];  // count of available tools per tier (wood is unlimited)
// ── Combat constants ─────────────────────────────────────────────
// Building HP: [House, Bakery, Wall, Tower, Farmland, Mine, Barracks, Forge, Road, Outpost, Gate]
const BLDG_HP_MAX    = [80, 30, 100, 120, 20, 50, 80, 50, 1, 120, 80];
const TC_HP_MAX      = 250;
...
const knightGroups = {};
for (let i = 1; i <= 9; i++) knightGroups[i] = new Set();
